...
(function($) {
   ...
   $(function(){
      // Pointer events for Win8 and WP8:
         ...
         $.firstArticle = ($('article').eq(0).length>0 ? $('article').eq(0) : undefined);
         ...
         //////////////////////////////////////////
         $.extend($, {
            UINavigationHistory: ($.firstArticle !== undefined ? ["#" + $.firstArticle.attr('id')] : [])
         });
   ...
   $.extend($, {
      applyBindings: function() {
         ...
         ///////////////////////////////////////////////////////////
         $('nav').isnt('#global-nav').each(function(ctx, idx) {
            var temp;
            if (window && window.jQuery && $ === window.jQuery) {
               temp = ctx;
               ctx = idx;
         ...
         $('article').each(function(ctx, idx) {
            ...
            if ($.firstArticle === undefined) {
               $.firstArticle = "#" + $(ctx);
               $.UINavigationHistory.push("#" + $(ctx).attr('id'));
            }
            // Prevent if splitlayout for tablets:
            if ($.body[0].classList.contains('splitlayout')) return;
